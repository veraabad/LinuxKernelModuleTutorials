...
{
    int status;
    pr_info("serdev_echo - Now I am in the probe function\n");
    
    serdev_device_set_client_ops(serdev, &serdev_echo_ops);
    status = serdev_device_open(serdev);
    if (status) {
        pr_err("serdev_echo - Error opening serial port\n");
        return -status;
    }
...
static void serdev_echo_remove(struct serdev_device *serdev)
{
    pr_info("serdev_echo - Now I am in the remove function\n");
    serdev_device_close(serdev);
}

static int __init my_init(void)
{
    pr_info("serdev_echo - Loading the driver...\n");
    if (serdev_device_driver_register(&serdev_echo_driver)) {
        pr_err("serdev_echo - Error: Could not load driver\n");
        return -1;
    }
    return 0;
}
...
MODULE_DESCRIPTION("A simple loopback driver for a UART port");
