...
static int __init my_init(void)
{
    ...
    pr_info("hello_cdev - Registered a characer device for Major %d starting with Minor %d\n", MAJOR(dev_nr), MINOR(dev_nr));

    my_class = class_create("my_class");
    if (!my_class) {
        pr_err("hello_cdev - Error could not create class my_class\n");
        status = ENOMEM;
        goto delete_cdev;
    }

    if (!device_create(my_class, NULL, dev_nr, NULL, "hello%d", 0)) {
        pr_err("hello_cdev - Error could not create device hello0\n");
        status = ENOMEM;
        goto delete_class;
    }
